---
page_title: "latitudesh_firewall_assignment Resource - latitudesh"
subcategory: ""
description: |-
  Firewall Assignment resource. Assigns a firewall to a server or to a virtual machine — set exactly one of server_id or virtual_machine_id.
---

# latitudesh_firewall_assignment (Resource)

The firewall assignment resource allows you to assign firewalls to servers or virtual machines within your [Latitude.sh](https://latitude.sh/) account. Set exactly one of `server_id` or `virtual_machine_id`.
...
A firewall can also be assigned to a virtual machine. A virtual machine can be assigned to at most one firewall.
...
- `server_id` (String) The ID of the server to assign the firewall to. Exactly one of `server_id` or `virtual_machine_id` must be set.
- `virtual_machine_id` (String) The ID of the virtual machine to assign the firewall to. Exactly one of `server_id` or `virtual_machine_id` must be set. A virtual machine can be assigned to at most one firewall.

### Read-Only

- `id` (String) Firewall assignment identifier
