...
resource "latitudesh_virtual_machine" "bastion_restored" {
  name      = "bastion-restored"
  backup_id = data.latitudesh_virtual_machine_backups.bastion.backups[0].id
}
```
...
- `plan` (String) The plan (ID or slug) for the virtual machine. Required unless `backup_id` is set, in which case the plan is inherited from the backup. Changing this forces a new resource.
